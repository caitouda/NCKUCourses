...
<%
    ...
    try {
        ...
        pstmt = conn.prepareStatement("select distinct CID from contest;");	
...
<%
    ...
    try {
        ...
        pstmt2 = conn2.prepareStatement("select distinct Ctitle from contest;");	
...
<%
    ...
    try {
        ...
        pstmt3 = conn3.prepareStatement("select distinct Starttime from contest;");	
...
<%
    ...
    try {
        ...
        pstmt4 = conn4.prepareStatement("select distinct Endtime from contest;");	
...
<%
	...
	ResultSet rs19 = null;
    try {
        conn19 = MySQLDBCon.getConn();
        pstmt19 = conn19.prepareStatement("select distinct Status from contest;");	
        rs19 = pstmt19.executeQuery();		
	    ResultSetMetaData md19 = rs19.getMetaData();
	    int columnCount19 = md19.getColumnCount(); 
	    while (rs19.next()) {    
	        for (int i = 1; i <= columnCount19; i++) {  
%> 
<option value="<%=rs19.getObject(i).toString()%>"><%=rs19.getObject(i).toString()%></option>
<%
            }   
	    }  
        } catch(Exception ex) {
...
<%
	...
	ResultSet rs20 = null;
    try {
        conn20 = MySQLDBCon.getConn();
        pstmt20 = conn20.prepareStatement("select distinct Access from contest;");	
        rs20 = pstmt20.executeQuery();		
	    ResultSetMetaData md20 = rs20.getMetaData();
	    int columnCount20 = md20.getColumnCount(); 
	    while (rs20.next()) {    
	        for (int i = 1; i <= columnCount20; i++) {  
%> 
<option value="<%=rs20.getObject(i).toString()%>"><%=rs20.getObject(i).toString()%></option>
<%
            }   
	    }  
        } catch(Exception ex) {
...
<%
	...
	ResultSet rs21 = null;
    try {
        conn21 = MySQLDBCon.getConn();
        pstmt21 = conn21.prepareStatement("select distinct SchoolName from contest;");	
        rs21 = pstmt21.executeQuery();		
	    ResultSetMetaData md21 = rs21.getMetaData();
	    int columnCount21 = md21.getColumnCount(); 
	    while (rs21.next()) {    
	        for (int i = 1; i <= columnCount21; i++) {  
%> 
<option value="<%=rs21.getObject(i).toString()%>"><%=rs21.getObject(i).toString()%></option>
<%
            }   
	    }  
        } catch(Exception ex) {
...
<select name="AC" id="AC" style="width:140px; ">
...
<select name="Try" id="Try" style="width:140px; ">
...
<button formaction="contest_Problem_all" method="post" type="submit">ALL</button>
<button formaction="contest_Problem_in" method="post" type="submit">IN</button>
<button formaction="contest_Problem_notin" method="post" type="submit">NOT IN</button>
<button formaction="contest_Problem_exists" method="post" type="submit">EXISTS</button>
<button formaction="contest_Problem_notexists" method="post" type="submit">NOT EXISTS</button>
</form>
<%
...
<button formaction="contest_Problem_aggregate" method="post" type="submit">AGGREGATE</button>
</form>
<%
