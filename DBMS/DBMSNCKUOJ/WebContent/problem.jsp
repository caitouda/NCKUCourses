...
<button onclick="window.location='insertproblem.jsp';">INSERT</button><br><br>
<%
    Connection conn7 = null;
	PreparedStatement pstmt7 = null;
	ResultSet rs7 = null;
	String sql7 = (String)request.getAttribute("sql7");
    ...
    if(exist == "true")
    {
        try {
	        ...
	        pstmt7 = conn7.prepareStatement(sql7);	
	        ...
	        while (rs7.next()) {    
	            out.print("<tr>");
	            for (int i = 1; i <= columnCount7; i++) {   
	                out.println("<th width=100>"+rs7.getObject(i).toString()); 
	                }   
	            PID[j] = rs7.getObject(1).toString();
%>  	            
	            <th width=100>
	              <a href="deleteByPID?PID=<%=PID[j] %>">刪除</a>
	              <a href="searchByPID?PID=<%=PID[j] %>">更新</a>
<%
                j++;
	            out.print("</tr>");
	            }  
...
<%
    ...
    try {
        ...
        out.print("<form action='updateproblem' method='post'>");
        out.print("<table border=2>");
        out.print("<tr>");
        ...
        while (rs8.next()) {    
            ...
            for (int i = 1; i <= columnCount8; i++) {   
%>
<th width=100><input type="text" name="<%=md8.getColumnName(i).toString()%>" value="<%=rs8.getObject(i).toString() %>"/>
<%        	
            }   
            out.print("</tr>");
        }  
